...
const SETTINGS_DEFAULTS = {
  ...
  defaults: { coins: 500, memory: 2048, cpu: 150, disk: 10240, servers: 2, backups: 1, databases: 1 },
...
function eggCatalog() {
  return [
    ...
    {
      name: 'Minecraft: Forge', category: 'Minecraft: Java', installer: 'forge',
      description: 'Modded Minecraft (Forge). Auto-downloads and runs the official Forge installer for the chosen version, then configures the startup automatically.',
      docker: MC_DOCKER, startup: MC_START, stopCommand: 'stop',
      variables: [
        { name: 'Minecraft Version', env: 'MINECRAFT_VERSION', default: 'latest', userEditable: true },
        { name: 'Forge Version', env: 'FORGE_VERSION', default: 'latest', userEditable: true },
      ],
    },
    {
      name: 'Minecraft: NeoForge', category: 'Minecraft: Java', installer: 'neoforge',
      description: 'Modded Minecraft (NeoForge). Auto-downloads and runs the official NeoForge installer, then configures the startup automatically.',
      docker: MC_DOCKER, startup: MC_START, stopCommand: 'stop',
      variables: [{ name: 'NeoForge Version', env: 'NEOFORGE_VERSION', default: 'latest', userEditable: true }],
    },
    {
      name: 'Minecraft: Spigot', category: 'Minecraft: Java', installer: 'none',
      description: 'Spigot/CraftBukkit (plugins). Spigot may only be distributed by building it yourself (BuildTools), so upload your built server.jar via SFTP. Tip: the Paper egg auto-installs and runs all Spigot/Bukkit plugins.',
      docker: MC_DOCKER, startup: MC_START, stopCommand: 'stop',
      variables: [],
    },
    {
      name: 'Sponge (SpongeVanilla)', category: 'Minecraft: Java', installer: 'sponge',
      description: 'SpongeVanilla modding platform. Auto-downloads the recommended SpongeVanilla build and accepts the EULA.',
      docker: MC_DOCKER, startup: MC_START, stopCommand: 'stop',
      variables: [{ name: 'Sponge Version', env: 'SPONGE_VERSION', default: 'latest', userEditable: true }],
    },
    {
      name: 'Minecraft: Bedrock Edition', category: 'Minecraft: Bedrock', installer: 'none',
      description: 'Official Bedrock Dedicated Server. Mojang offers no public download API, so download it from minecraft.net and upload the files via SFTP, then start. (Linux host.)',
    ...
    },
    {
      ...
      description: 'Terraria dedicated server. Download the server zip from terraria.org and upload the files via SFTP, set the binary name, and start.',
...
function migrateUsers() {
  ...
  for (const u of backend.all('users')) {
    ...
    if (!u.resources || typeof u.resources !== 'object') {
    ...
    } else if (u.resources.databases === undefined) {
      ...
      patch.resources = {
        ...u.resources,
        backups: Math.max(u.resources.backups || 0, allocBackups, d.backups || 0),
        databases: Math.max(allocDbs, d.databases || 0),
      };
    }
...
function migrateServers() {
  for (const s of backend.all('servers')) {
    ...
    if (!s.featureLimits || typeof s.featureLimits !== 'object')
      patch.featureLimits = { databases: 1, backups: 1, allocations: 5 };
